...
public class MainActivity extends AppCompatActivity {
    ...
    public void TopicGetter()
    {
        ...
        GetTopicsRequest.Builder mGetTopicsRequest = new GetTopicsRequest.Builder(mContext).setSdkName("com.example.adtech");
    ...
    OutcomeReceiver mCallback = new OutcomeReceiver<GetTopicsResponse, GetTopicsException>() {
        ...
        public void onResult(@NonNull GetTopicsResponse result)
        {
            for(int i = 0; i < result.getTopics().size(); i++){
                Log.i("Topic", mTaxonomy[result.getTopics().get(i).getTopicId()-1]);
                if(results.isEnabled())
                {
                    //Receives the Topic ID and pulls the corresponding entry from the Topics Taxonomy
                    results.setText(mTaxonomy[result.getTopics().get(i).getTopicId()-1]);
                }
            }
        ...
        public void onError(@NonNull GetTopicsException error)
        {
            ...
            if(results.isEnabled())
            {
                results.setText("Returned An Error");
            }
        }
    };
    ...
    public static String[] mTaxonomy = new String[]
        {
            "/Arts & Entertainment",
