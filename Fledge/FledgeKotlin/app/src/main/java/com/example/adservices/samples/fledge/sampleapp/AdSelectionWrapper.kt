...
class AdSelectionWrapper(
      ...
      object : FutureCallback<Void?> {
        override fun onSuccess(unused: Void?) {
          ...
          if (isTestableVersion(8, 9)) {
            ...
            Log.w(TAG, "Unsupported SDK Extension: Event reporting requires 8, skipping")
          } else {
  ...
  fun reportEvent(
    ...
    val request =
      ReportEventRequest.Builder(adSelectionId, eventKey, eventData, reportingDestinations).build()
    Futures.addCallback(
  ...
  fun overrideAdSelection(
    ...
    val request =
      AddAdSelectionOverrideRequest(adSelectionConfig, decisionLogicJS!!, trustedScoringSignals!!)
    Futures.addCallback(
      ...
      AdSelectionConfig.Builder()
        ...
        .setPerBuyerSignals(
          buyers
            .stream()
            .collect(
              Collectors.toMap({ buyer: AdTechIdentifier -> buyer }, { AdSelectionSignals.EMPTY })
            )
        )
