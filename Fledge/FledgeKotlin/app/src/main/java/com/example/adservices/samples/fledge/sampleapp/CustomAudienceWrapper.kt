...
class CustomAudienceWrapper(private val executor: Executor, context: Context) {
  ...
  fun joinFilteringCa(
    ...
    try {
      joinCustomAudience(
        getCustomAudienceBuilder(name, buyer, biddingUri,
          getAdDataBuilder(renderUri).setAdFilters(filters).setAdCounterKeys(adCounterKeys).build(),
  ...
  private fun getCustomAudienceBuilder(
          ...
          expiry: Instant): CustomAudience.Builder {
