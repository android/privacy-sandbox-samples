...
class MainActivity : AppCompatActivity() {
  ...
  private fun setupCASwitches(
    ...
    val buyer: AdTechIdentifier = AdTechIdentifier.fromString(biddingUri.host!!)
    // Shoes
    ...
    binding.freqCapCaSwitch.setOnCheckedChangeListener { _, isChecked ->
      ...
      val keyedFrequencyCapImpression =
        KeyedFrequencyCap.Builder(adCounterKey, 2, Duration.ofSeconds(10)).build()

      // Caps is exceeded after 1 click event
      val keyedFrequencyCapClick =
        KeyedFrequencyCap.Builder(adCounterKey, 1, Duration.ofSeconds(10)).build()
