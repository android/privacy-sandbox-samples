...
public class MainActivity extends AppCompatActivity {
  ...
  private void toggleOverrideSwitch(CompoundButton buttonView, boolean isChecked) {
    if (isChecked) {
      ...
      try {
        String baseUri =
            getIntentOrError(BASE_URL_INTENT, eventLog, MISSING_FIELD_STRING_FORMAT_RESTART_APP);
        ...
        adWrapper.resetAdSelectionConfig(
            Collections.singletonList(mBuyer), mSeller, mScoringLogicUri, mTrustedDataUri);
  ...
  private void setupCASwitches(
    ...
    binding.freqCapCaSwitch.setOnCheckedChangeListener(
        (compoundButton, isChecked) -> {
          ...
          KeyedFrequencyCap keyedFrequencyCapImpression =
              new KeyedFrequencyCap.Builder(adCounterKey, 2, Duration.ofSeconds(10)).build();

          // Caps is exceeded after 1 click event
          KeyedFrequencyCap keyedFrequencyCapClick =
              new KeyedFrequencyCap.Builder(adCounterKey, 1, Duration.ofSeconds(10)).build();
    ...
    AdTechIdentifier buyerServer =
            AdTechIdentifier.fromString(serverAuctionBiddingUri.getHost());
        ...
        (compoundButton, isChecked) -> {
          ...
          if (isChecked) {
            ...
            caWrapper.leaveCa(
                HATS_CA_NAME,
                context.getPackageName(),
                AdTechIdentifier.fromString(biddingUri.getHost()),
                eventLog::writeEvent);
          }
        });
  }
        ...
        (l) -> {
          try {
            ...
            adSelectionWrapper.updateAdCounterHistogram(
                adSelectionId, FrequencyCapFilters.AD_EVENT_TYPE_CLICK, eventLog::writeEvent);
          } catch (NumberFormatException e) {
            Log.e(TAG, String.format("Error while parsing the ad selection id: %s", e));
  ...
  private void useOverrides(
    ...
    adSelectionWrapper.overrideAdSelection(
        eventLog::writeEvent, decisionLogicJs, trustedScoringSignals);
  ...
  private boolean isAuctionServerSetupReady(String reason) {
    ...
    StringBuilder errMsgBuilder = new StringBuilder(String.format("For %s, you have to pass intent(s): ", reason));
